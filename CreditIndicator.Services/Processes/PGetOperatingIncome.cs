...
{
    public class PGetOperatingIncome
    {
        ...
        {
            var ItemId = new int();
            var TotalOperatingIncome = new decimal();
            ...
            try
            {
                ...
                {// find Val list for records and sums there value
                    var OperatingIncomeRecordsList = DBModelHelper.GetItemValue(ItemId, StartDate, EndDate);
                    TotalOperatingIncome = OperatingIncomeRecordsList.Sum(item => item.ItemValue);
                }
            }
            catch (Exception ex)
            {
                logger.Handle(ex.ToString(), executionStatus);
                throw new ApplicationException("PGetOperatingIncome :: Exception occured", ex);
            }
